...
from openchem.utils.metrics import logP, sa_score, logP_pen, reward_penalized_log_p
...
def get_logP(target, smiles):
    if len(smiles) > 0:
        scores = np.array(logP(smiles, return_mean=False))
        ind = np.where((scores >= 0.0) & (
                scores <= 5.0))[0]
        return len(ind)/len(smiles)
    else:
        return 0.0
...
from openchem.utils.rl_utils import qed_reward_fn, logp_range_rew, logp_pen_rew

max_atom_bonds = [4, 3., 2., 1., 5., 6., 1., 1., 1.]

my_loss = PolicyGradientLoss(reward_fn=qed_reward_fn, critic=reward_penalized_log_p, tokens=None,
                             fn=logp_pen_rew, gamma=0.97,
